    ...
    <div>
        <el-container>
            ...
            <el-header class="user-header" height="300">
                ...
                <el-tag type="danger">{{getCount}}</el-tag>
            </el-header>
...
<script>
    //引入api方法
    import {firendAll,groups} from "../../api/fans";
    export default {
        name: "tags",
        data() {
            return {
        ...
        computed:{
            getCount(){
                ...
                let count = 0;
                if(table.length == 0)return count
                for(var k in table){
                    count = table[k]['allCount'] + count
                }
                return count
            }
        },
        methods:{
            ...
            search(currentPage) {
                ...
                this.keyWords['excel'] = 0
                firendAll(this.keyWords).then(res => {
            ...
            excel(){
                ...
                if(this.excelData.length>0 && flag){
                    ...
                    require.ensure([], () => {
                        ...
                        const tHeader = ['id', '分组','微信号码', '粉丝数']; //将对应的属性名转换成中文

                        const filterVal = ['id', 'groups','user_no', 'allCount'];
                        const list = this.excelData;
                        //把data里的tableData存到list
                        const data = this.formatJson(filterVal, list);
                        export_json_to_excel(tHeader, data, '监控粉丝统计列表');
                    })
                }
                else{
                    this.keyWords['excel'] = 1
                    firendAll(this.keyWords).
                    then(res=>{
                        if(res.code == 0){
                            this.Load = false;
                            require.ensure([], () => {
